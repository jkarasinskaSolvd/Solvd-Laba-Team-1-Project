...
-- ---ptimising logistics for order fulfilment`.`Address`
      -- -----------------------------------------------------
      ...
      -- -----------------------------------------------------
      -- Table `Optimising logistics for order fulfilment`.`WarehouseAllowedTransport`
      -- -----------------------------------------------------
      CREATE TABLE IF NOT EXISTS `Optimising logistics for order fulfilment`.`WarehouseAllowedTransport` (
        `warehouse_allowed_transport_id` BIGINT(20) NOT NULL,
        `transport_type_warehouse` ENUM('Truck', 'Train', 'Airplane') NOT NULL,
        CONSTRAINT `warehouse_allowed_transport_id`
          FOREIGN KEY (`warehouse_allowed_transport_id`)
          REFERENCES `Optimising logistics for order fulfilment`.`Warehouses` (`id`)
          ON DELETE CASCADE
          ON UPDATE CASCADE)
      ENGINE = InnoDB;
      ...
      CREATE TABLE IF NOT EXISTS `Optimising logistics for order fulfilment`.`OrderItems` (
        ...
        `product_id_order_items` BIGINT(20) NOT NULL,
        `quantity_order_items` INT NOT NULL CHECK (quantity_order_items > 0),
        INDEX `product_id_order_items_idx` (`product_id_order_items` ASC) VISIBLE,
        CONSTRAINT `order_id`
          FOREIGN KEY (`order_id`)
          REFERENCES `Optimising logistics for order fulfilment`.`Orders` (`id`)
          ON DELETE CASCADE
          ON UPDATE CASCADE,
        CONSTRAINT `product_id_order_items`
          FOREIGN KEY (`product_id_order_items`)
          REFERENCES `Optimising logistics for order fulfilment`.`Products` (`id`)
          ON DELETE CASCADE
          ON UPDATE CASCADE)
      ENGINE = InnoDB;
